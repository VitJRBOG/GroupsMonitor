...
class NewPost:
    ...
    def get_posts(self, sender, vk_admin_session,
                  subject_data, subject_section_data):
        sender += " -> Get post"

        try:
            owner_id = int(subject_data["owner_id"])
            count = int(subject_section_data["posts_count"])
            post_filter = str(subject_section_data["filter"])
            ...
            return self.get_posts(sender, vk_admin_session,
                                  subject_data, subject_section_data)
        ...
        try:
            ...
            def get_signature(sender, vk_admin_session, item):
                ...
                try:
                    ...
                    logger.exception_handler(sender, var_except)
                    return get_signature(sender, vk_admin_session, item)
    ...
    def send_message(self, sender, vk_bot_session,
                     subject_data, subject_section_data, message_object):
        sender += " -> Send message"

        try:
            peer_id = subject_section_data["send_to"]
            ...
            logger.exception_handler(sender, var_except)
            return self.send_message(sender, vk_bot_session, subject_data,
    ...
    def new_post(self, sender, sessions_list, subject_data,
        ...
        try:
            ...
            vk_admin_session = sessions_list["admin_session"]
            # vk_bot_session = sessions_list["bot_session"]
            ...
            logger.exception_handler(sender, var_except)
            return self.new_post(sender, sessions_list,
                                 subject_data, subject_section_data)


class NewAlbumPhoto:

    def get_photo(self, sender, vk_admin_session,
                  subject_data, subject_section_data):
        sender += " -> Get photo"

        try:
            settings = subject_section_data
            ...
            logger.exception_handler(sender, var_except)
            return self.get_photo(sender, vk_admin_session,
                                  subject_data, subject_section_data)
    ...
    def new_album_photo(self, sender, sessions_list,
                        subject_data, subject_section_data):
        ...
        try:
            ...
            response = self.get_photo(sender, vk_admin_session,
                                      subject_data, subject_section_data)
            ...
            logger.exception_handler(sender, var_except)
            return self.new_album_photo(sender, sessions_list,
                                        subject_data, subject_section_data)


class NewVideo:

    def get_videos(self, sender, vk_admin_session,
                   subject_data, subject_section_data):
        ...
        try:
            ...
            owner_id = int(subject_data["owner_id"])
            count = int(subject_section_data["video_count"])
            ...
            response = vk_admin_session.method("video.get", values)

            return response

        except Exception as var_except:
            logger.exception_handler(sender, var_except)
            return self.get_videos(sender, vk_admin_session,
                                   subject_data, subject_section_data)
    ...
    def make_message(self, sender, vk_admin_session, item):
        ...
        try:
            ...
            def get_signature(sender, vk_admin_session, item):
                ...
                try:
                    if str(item["owner_id"])[0] == "-":

                        post_signature = "New video: "
                        ...
                        author_url = "*id" + str(item["owner_id"]) +\
                            " (" + author_full_name + ")"
            ...
            # Функция возвращает описание к видео
            def get_description(sender, item):
                sender += " -> Get text"
                ...
                try:
                    ...
                    logger.exception_handler(sender, var_except)
                    return get_description(sender, item)
            ...
            def get_url(sender, item):
                ...
                try:
                    ...
                    post_url = "https://vk.com/video" + id_post
            ...
            video_signature = get_signature(sender,
                                            vk_admin_session,
                                            item)
            video_description = get_description(sender, item)
            video_url = get_url(sender, item)
            video_attachments = get_attachments(sender, item)

            mes_long_text = "...\n[long text]"

            video_length = len(video_signature + "\n\n" +
                               video_description +
                               mes_long_text + "\n\n" +
                               video_url)
            ...
            if video_length > limit_symbols:
                count_symbols = video_length -\
                    (video_length - limit_symbols) - 1
                video_description = video_description[0:count_symbols]

                message = video_signature + "\n\n" +\
                    video_description +\
                    mes_long_text + "\n\n" +\
                    video_url
            else:
            ...
            return message, video_attachments
    ...
    def new_video(self, sender, sessions_list,
                  subject_data, subject_section_data):

        sender += " -> Notificator -> New video"

        vk_admin_session = sessions_list["admin_session"]
        # vk_bot_session = sessions_list["bot_session"]

        response = self.get_videos(sender, vk_admin_session,
                                   subject_data, subject_section_data)

        return response


class NewPhotoComment():

    def get_comments(self, sender, vk_admin_session,
                     subject_data, subject_section_data):
        sender += " -> Get comments"
        ...
        try:
            ...
            owner_id = int(subject_data["owner_id"])
            count = int(subject_section_data["comment_count"])
            ...
            response = vk_admin_session.method("photos.getAllComments", values)

            return response

        except Exception as var_except:
            logger.exception_handler(sender, var_except)
            return self.get_comments(sender, vk_admin_session,
                                     subject_data, subject_section_data)
        ...
        try:
            ...
            def get_signature(sender, vk_admin_session, item, subject_data):
                ...
                try:
                    ...
                    def get_subject_name(sender, vk_admin_session,
                                         subject_data):
                        ...
                        try:
                            ...
                            logger.exception_handler(sender, var_except)
                            return get_subject_name(sender, vk_admin_session,
                                                    subject_data)
                    ...
                    if str(item["from_id"])[0] == "-":
                        ...
                        subject_name = get_subject_name(sender,
                                                        vk_admin_session,
                                                        subject_data)
                    ...
                    logger.exception_handler(sender, var_except)
                    return self.get_signature(sender, vk_admin_session,
                                              item, subject_data)
            ...
            def get_text(sender, item):
                ...
                try:
                    ...
                    logger.exception_handler(sender, var_except)
                    return self.get_text(sender, item)

            def get_url(sender, item, subject_data):
                ...
                try:
                    ...
                    id_photo = str(owner_id) + "_" + str(item["pid"])

                    photo_url = "https://vk.com/photo" + id_photo

                    return photo_url

                except Exception as var_except:
                    logger.exception_handler(sender, var_except)
                    return self.get_url(sender, item, subject_data)
            ...
            def get_attachments(sender, item):
                ...
                try:
                    ...
                    logger.exception_handler(sender, var_except)
                    return self.get_attachments(sender, item)
            ...
            photo_url = get_url(sender, item, subject_data)
            ...
            logger.exception_handler(sender, var_except)
            return self.make_message(sender, vk_admin_session,
                                     item, subject_data)
    ...
    def new_photo_comment(self, sender, sessions_list,
                          subject_data, subject_section_data):
        ...
        response = self.get_comments(sender, vk_admin_session,
                                     subject_data, subject_section_data)

        return response


class NewVideoComment():
    ...
    def get_videos(self, sender, vk_admin_session,
        ...
        try:
            ...
            owner_id = int(subject_data["owner_id"])
            videos_count = int(subject_section_data["videos_count"])
    ...
    def get_comments(self, sender, vk_admin_session, video,
                     subject_data, subject_section_data):
        ...
        try:
            ...
            values = {
                ...
                "video_id": video["id"],
                "count": subject_section_data["comment_count"],
                "sort": "desc"
            }

            response = vk_admin_session.method("video.getComments", values)

            return response

        except Exception as var_except:
            logger.exception_handler(sender, var_except)
            return self.get_comments(sender, vk_admin_session, video,
                                     subject_data, subject_section_data)
        ...
        try:
            ...
            def get_url(sender, item):
                ...
                try:
                    ...
                    owner_id = item["video_owner_id"]

                    id_video = str(owner_id) + "_" + str(item["video_id"])

                    video_url = "https://vk.com/video" + id_video
    ...
    def new_video_comment(self, sender, sessions_list, post,
                          subject_data, subject_section_data):
        ...
        response = self.get_comments(sender, vk_admin_session, post,
                                     subject_data, subject_section_data)
...
class NewTopicMessage:
    ...
    def get_topics(self, sender, vk_admin_session,
                   subject_data, subject_section_data):
        sender += " -> Get topics"
        ...
        try:
            ...
            logger.exception_handler(sender, var_except)
            return self.get_topics(sender, vk_admin_session,
                                   subject_data, subject_section_data)

    def checking_existence(self, sender, subject_data, subject_section_data,
                           response):
        ...
        try:
            ...
            topics_subject = copy.deepcopy(subject_section_data["topics"])
            ...
            if len(topics_subject) > 0:
                ...
                while i < len(response):
                    ...
                    while j < len(topics_subject):
                        ...
                        if response_item["id"] ==\
                           topics_subject_item["id"]:
                            not_exist = False
                            if subject_section_data["topics"][j]["title"] !=\
                               response_item["title"]:
                                subject_section_data["topics"][j]["title"] =\
                                    response_item["title"]
                            break
                        else:
                            not_exist = True
                    ...
                    if not_exist:
                        ...
                        subject_section_data["topics"].append(copy.deepcopy(topic_values))
                ...
                while i < len(topics_subject):
                    ...
                    if not_exist:
                        subject_section_data["topics"].pop(i)
            ...
            logger.exception_handler(sender, var_except)
            return self.checking_existence(sender, subject_data,
                                           subject_section_data, response)
    ...
    def get_comments(self, sender, vk_admin_session,
        ...
        try:
            ...
            topic_checker_settings = subject_section_data

            i = 0

            while i < len(subject_section_data["topics"]):

                topic_id = int(subject_section_data["topics"][i]["id"])
                ...
                comments_values = {
                    "owner_id": subject_data["owner_id"],
                    "topic_id": topic_id,
                    "topic_title": subject_section_data["topics"][i]["title"],
                    "last_date": subject_section_data["topics"][i]["last_date"],
    ...
    def new_topic_message(self, sender, sessions_list, subject_data,
        ...
        try:
            ...
            response = self.get_topics(sender, vk_admin_session,
                                       subject_data, subject_section_data)
            subject_data = self.checking_existence(sender, subject_data,
                                                   subject_section_data,
                                                   response)
            list_response = self.get_comments(sender, vk_admin_session,
                                              subject_data,
                                              subject_section_data)
            ...
            logger.exception_handler(sender, var_except)
            return self.new_topic_message(sender, sessions_list,
                                          subject_data, subject_section_data)
...
class NewPostComment():
    ...
    def get_posts(self, sender, vk_admin_session,
        ...
        try:
            ...
            owner_id = int(subject_data["owner_id"])
            count = int(subject_section_data["posts_count"])
            post_filter = subject_section_data["filter"]
    ...
    def get_comments(self, sender, vk_admin_session, post,
                     subject_data, subject_section_data):
        ...
        try:
            ...
            logger.exception_handler(sender, var_except)
            return self.get_comments(sender, vk_admin_session, post,
                                     subject_data, subject_section_data)
    ...
    def new_post_comment(self, sender, sessions_list, post,
                         subject_data, subject_section_data):
